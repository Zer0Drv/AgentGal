...
EPISODE_CLOSURE_DETECTOR = r"""<role>
看最近的聊天记录，从给定名单里找出哪些人最近的一段互动已经结束、最后活跃在哪一回合。
</role>

<inputs>
- <candidates>：需要判断的角色名单（agent_name → 显示名），只判断这些人。
- <recent_history>：近期对话，每条带 `[turn=N]` 标记。
</inputs>

<how_to_judge>
对名单里每个人，看他最近这段互动是否已经结束。结束有两种典型情况，任一成立都算结束：

A. 人离开了：说了告别 / 下线 / 回去；之后的 turn 里没再出现（没说话、没被提到、没明显行动）；场景发生物理或时间切换，这个人不在新场景里。

B. 事情聊完了：原来那件事已经自然收尾（得出结论、说完告一段落、情绪落定）；接下来大家转去做另一件事或另一个话题，核心目的明显换了；或者情绪场显著切换（轻松→认真 / 亲近→疏离 / 日常→冲突）。这种情况下即使这个人还在场，之前那段也算结束。

如果不属于上面两种，就是还在进行中：
- 话题还在延续，哪怕措辞变化、有过渡词（「对了」「还有」）
- 只是补充、回应、安慰、确认、收尾动作
- 本轮沉默陪伴、在场听别人说话
</how_to_judge>

<turn_rule>
对判为"已结束"的人，填这段已结束的互动里，这个人最后一次活跃（说话 / 明确行动 / 明确被描写在场景里）的 turn 号。
</turn_rule>

<fallback>
拿不准时，当作"进行中"，在输出里省略这个角色（宁可下一轮再判）。
</fallback>
...
例 3：事情聊完了，人还在
- 输入大意：candidates 只有 mitsuki；turn=35 两人把误会说开、情绪落定；turn=36 起玩家转去问明天的安排，mitsuki 也切到日程话题。
- 预期输出：{"mitsuki": 35}

例 4：有人走、有人话题继续
- 输入大意：candidates 包含 chenxiao 和 guyining；chenxiao 从 turn=39 起没再出现，guyining 从 turn=41 起还在和玩家谈同一件工作上的事。
- 预期输出：{"chenxiao": 39}

例 5：多人分别闭合于不同 turn
- 输入大意：candidates 包含 chenxiao 和 guyining；turn=50 chenxiao 先告辞离开，之后没再出现；guyining 留下来继续和玩家把手上的工作谈完，turn=53 两人把结论说定、情绪落地，turn=54 起玩家转去讨论晚饭安排。
- 预期输出：{"chenxiao": 50, "guyining": 53}
</examples>
