...
以**场景**为最小单位组织记忆，而非以分钟为单位。

## 规则

1. **按场景合并**：同一地点的连续互动合并为一条。判断标准：地点相同 + 时间连续 + 话题连贯。例如"教室收拾书包时的两段对话"应合为一条。
2. **时间粒度放粗**：用"放学后"、"傍晚"、"午休"等自然时段描述。可用"场景标签"代替时间戳，如「放学后 · 教室→商店街」。
3. **保留事实，压缩修辞**：不丢失任何事实、承诺、情感变化，但压缩冗余的氛围描写和重复信息。
4. **不要添加新内容**：只整理已有内容，不推测、不补充。
5. **保持角色视角**：这是 {agent_name} 的记忆，保持其主观视角，保留其感受和想法，风格一致。
6. **按时间顺序排列**：从最早到最近，保持时间线清晰。
