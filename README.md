...
每轮角色回应后，页面会给出几个可选行动。你可以点击选项继续，也可以自己输入完全不同的内容。
...
页面提供统一的档案抽屉，用档位管理存档、读档和重开。

- **新建存档**：生成一个新的档位，保存当前世界线。
- **覆盖**：把当前世界线写入指定档位。
- **读取**：恢复过去的世界线。
- **重开**：清空当前进度，从故事模板重新开始。
...
页面顶部的历史按钮可以按游戏内日期跳回旧回合，也可以搜索消息内容并跳到匹配回合。长线游玩时，可以用它找回早先的约定、误会或角色提到过的细节。
