...
- narrator / choices / consolidation / character_factory / offstage_synthesizer 都支持各自的独立 LLM 配置，未设置时逐级回退（`CHARACTER_FACTORY_LLM_*` 回退 narrator，`OFFSTAGE_SYNTH_LLM_*` 回退 consolidation）

### `config.toml`

- 放运行时策略参数，例如 Agent temperature、超时、向量检索权重
- `[history]` 中的 `history_high` / `history_low` 控制多轮消息高低水位截断
- `[offstage]` 中的 `catchup_threshold_days` 控制离场追补阈值（游戏内天数）
