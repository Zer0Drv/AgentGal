...
- `[history]` 中的 `history_high` / `history_low` 控制多轮对话高低水位截断（以 distinct turn 数为单位，基于 turn 号锚定，存于 `.history_window_state.json` 的 `start_turn` 字段）；`raw_scan_turns` 限制 narrator 路由 / 角色运行 / 选项生成读取 raw 历史时回溯的 turn 数（整理流程按 turn 切片，不受此限制）
